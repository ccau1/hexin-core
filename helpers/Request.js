...
module.exports.fetch = (req, url, opt) => {
  return new Promise((resolve, reject) => {
    ...
    if (!opt.headers['X-Request-ID'] || !opt.headers['X-Client-Authorization']) {
      if (req.headers['x-request-id'] && req.headers['x-client-authorization']) {
        opt.headers['X-Request-ID'] = req.headers['x-request-id'];
        opt.headers['X-Client-Authorization'] = req.headers['x-client-authorization'];
      }
    }
